...
| 集群 | ConfigMap `minitube`、Secret `minitube`（生产 ns `minitube` / 测试 ns `minitube-test` 各一份）；`DATABASE_URL` 在容器启动命令里用 `POSTGRES_PASSWORD` 拼出来 |
...
| 变量 | 缺省 | 用途 |
| --- | --- | --- |
| `APP_ENV` | `dev` | `dev` / `test` / `prod`。`test` 时强制关闭 ctc，管理页锁定分流 |
| `HTTP_ADDR` | `:8080` | HTTP 监听（容器内；测试宿主机映射 8081） |
...
| `PUBLIC_BASE_URL` | `http://127.0.0.1:8080` | 对外媒体/链接基址。生产 `https://minitube.19121122.xyz`，测试 `https://minitube-test.19121122.xyz` |
| `UPLOAD_DIR` | `./data/uploads` | 上传与点播文件。集群 ConfigMap 为 `/data/uploads` |
| `RTMP_BASE_URL` | `rtmp://127.0.0.1:1935/live` | 创建直播时下发的推流前缀。生产 `…:1935/live`，测试 `…:1936/live` |
...
| `ADMIN_SETUP_URL` | `http://192.168.43.111:8080/admin/setup` | 管理页里局域网维护链接。测试为 `:8081` |
